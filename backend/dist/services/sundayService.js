...
export class SundayService {
    static async ensureSundayServiceExists() {
        try {
            ...
            if (today.getDay() !== 0) {
                console.log(`ℹ️  Not Sunday (${today.getDay()}), skipping Sunday service check`);
            ...
            const existingService = await db
                .select()
                .from(events)
                .where(and(eq(events.date, todayStr), eq(events.eventType, 'sunday_service')));
            ...
            if (existingService.length === 0) {
                const [newEvent] = await db
                    ...
                    .returning();
                console.log(`✅ Auto-created Sunday service for ${todayStr}`);
                return { created: true, event: newEvent };
            }
            console.log(`ℹ️  Sunday service already exists for ${todayStr}`);
    ...
    static startDailyCheck() {
        ...
        cron.schedule('0 6 * * *', async () => {
            console.log('🕒 Running daily Sunday service check...');
            await this.ensureSundayServiceExists();
        });
        console.log('✅ Sunday service daily check scheduled (6:00 AM daily)');
    }
